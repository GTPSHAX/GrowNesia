...
// Menghubungkan header yang dibutuhkan 
#include <string>
...
public:
	...
	void CreatePacket(ENetPeer* peer) {
		...
		if (peer != nullptr && peer->state == ENET_PEER_STATE_CONNECTED) {
			...
			enet_host_flush(peer->host); // Pastikan peer masih valid sebelum flush
		}
		else {
			enet_packet_destroy(packet); // Hancurkan paket jika peer sudah tidak valid
		}
	}
};